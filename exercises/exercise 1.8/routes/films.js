...
router.get('/:id',(req,res) => {
    ...
    const indexFound = films.findIndex( film => film.id === Number(req.params.id) ); // on va créer la variable film directement dans la fonction et on va dire que
                                                                            // son id est celui entré dans l'URL
    if(indexFound < 0) return res.sendStatus(404);
    return res.json(films[indexFound]);
});
...
router.get('/',(req,res) =>{
    ...
    return res.json(orderFilm ?? films); // on renvoie le filtre et s'il est undefined, on renvoie le tableau par défaut
});
...
router.delete('/:id',(req,res) => {
    ...
    const filmID = films.findIndex(film => film.id === Number(req.params.id)); // étant donné la vérification du type par le triple égal, on modifie le type de l'ID de String à Number 
...
router.patch('/:id',(req,res) => {
    ...
    const indexFilm = films.findIndex(f => f.id === Number(req.params.id)); // vérification de l'ID (le triple égal vérifie le type en plus de la valeur => double égal = la valeur)
...
router.put('/:id',(req,res) =>{
    ...
    const findId = films.findIndex(f =>f.id === Number(req.params.id)); // si l'on trouve l'ID, on met à jour, sinon on ajoute
